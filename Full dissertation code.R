...
topN_genus_per_location <- topN_genus_per_location %>%
  left_join(distinct(samples_metadata, Catchment, Location, Altitude),
            by = c("Catchment","Location")) %>%
  group_by(Catchment) %>%
  mutate(Location = fct_reorder(Location, Altitude, .desc = FALSE)) %>%
  ungroup()
...
hayle_plot <- topN_genus_per_location %>%
  ...
  scale_x_continuous(labels = percent_format(accuracy = 1)) +
  theme_bw() +
  labs(title = "A", x = "Mean relative abundance", y = "Genus")
...
red_plot <- topN_genus_per_location %>%
  ...
  scale_x_continuous(labels = percent_format(accuracy = 1)) +
  theme_bw() +
  labs(title = "B", x = "Mean relative abundance", y = "Genus")
...
carnon_plot <- topN_genus_per_location %>%
  ...
  scale_x_continuous(labels = percent_format(accuracy = 1)) +
  theme_bw() +
  labs(title = "C", x = "Mean relative abundance", y = "Genus")
...
cober_plot <- topN_genus_per_location %>%
  ...
  scale_x_continuous(labels = percent_format(accuracy = 1)) +
  theme_bw() +
  labs(title = "D", x = "Mean relative abundance", y = "Genus")
